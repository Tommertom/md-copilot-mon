...
const currentSessionFolderEl = document.getElementById(
...
async function loadDiff(sessionId) {
  ...
  try {
    const response = await fetch(
      `/api/git-diff?sessionId=${encodeURIComponent(sessionId)}`,
    );
    if (!response.ok) {
    ...
    currentSessionFolderEl.textContent =
      typeof data.diffDirectory === "string" ? data.diffDirectory : "";
...
async function selectSession(sessionId) {
  ...
  currentSessionEl.textContent =
    nextSession?.title || nextSession?.directory || "Session";
  const previousCwd =
    typeof previousSession?.workspace?.cwd === "string"
      ? previousSession.workspace.cwd.trim()
      : "";
  const nextCwd =
    typeof nextSession?.workspace?.cwd === "string"
      ? nextSession.workspace.cwd.trim()
      : "";
  if (
    previousSession &&
